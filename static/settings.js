...
let widgetImages  = []; // array of {id, dataUrl} up to 3
...
function renderBgGrid() {
    ...
    if (bgImages.length < 3) {
        ...
        fileInput.addEventListener('change', async e => {
            ...
            const r = await fetch('/api/images/upload?type=bg', { method: 'POST', body: form });
            if (!r.ok) {
...
function renderProfileGrid() {
    ...
    profileImages.forEach((img, i) => {
        ...
        removeBtn.addEventListener('click', async e => {
            e.stopPropagation();
            await fetch('/api/images/' + profileImages[i].filename, { method: 'DELETE' });
    ...
    if (profileImages.length < 5) {
        ...
        fileInput.addEventListener('change', async e => {
            ...
            const form = new FormData();
            form.append('image', file);
            const r = await fetch('/api/images/upload?type=profile', { method: 'POST', body: form });
            if (!r.ok) {
                const err = await r.json();
...
function renderWidgetImageGrid() {
    ...
    widgetImages.forEach((img, i) => {
        ...
        removeBtn.addEventListener('click', async e => {
            e.stopPropagation();
            await fetch('/api/images/' + widgetImages[i].filename, { method: 'DELETE' });
    ...
    if (widgetImages.length < 3) {
        ...
        fileInput.addEventListener('change', async e => {
            ...
            const form = new FormData();
            form.append('image', file);
            const r = await fetch('/api/images/upload?type=widget', { method: 'POST', body: form });
            if (!r.ok) {
                const err = await r.json();
                showUploadError('widget-img-upload-error', err.error || 'upload failed');
                return;
            }
            const record = await r.json();
            widgetImages.push(record);
            renderWidgetImageGrid();
            await saveSettings();
        });
        slot.appendChild(fileInput);

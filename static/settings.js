...
function renderTheme() {
  ...
  THEMES.forEach((theme) => {
    ...
    btn.className =
      "option-btn" + (currentTheme === theme.id ? " selected" : "");
...
function renderFonts() {
  renderFontSelect("font-latin-select", FONTS_LATIN, fontLatin, (v) => {
    fontLatin = v;
  });
...
function processUpload(file, errorElId, drawFn, format = "jpeg") {
  return new Promise((resolve) => {
    if (file.size > UPLOAD_MAX_FILE_BYTES) {
      showUploadError(
        errorElId,
        `file too large — max 10MB (this file is ${(file.size / 1024 / 1024).toFixed(1)}MB)`,
      );
      return resolve(null);
    }
      ...
      imgEl.onload = () => {
        ...
        if (format === "png") {
          ...
          if (dataUrl.length > UPLOAD_MAX_OUTPUT_CHARS) {
            showUploadError(
              errorElId,
              "image too large after processing — try a smaller file",
            );
            return resolve(null);
          }
        ...
        showUploadError(
          errorElId,
          "image too large after compression — try a smaller or lower-res file",
        );
        resolve(null);
      };
...
function renderBgGrid() {
  ...
  bgImages.forEach((img, i) => {
    ...
    removeBtn.addEventListener("click", async (e) => {
      e.stopPropagation();
      await fetch("/api/images/" + bgImages[i].filename, { method: "DELETE" });
      bgImages.splice(i, 1);
      renderBgGrid();
      await saveSettings();
    });
  ...
  if (bgImages.length < 3) {
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      const r = await fetch("/api/images/upload?type=bg", {
        method: "POST",
        body: form,
      });
      if (!r.ok) {
        const err = await r.json();
        showUploadError("bg-upload-error", err.error || "upload failed");
        return;
      }
      const record = await r.json();
      bgImages.push(record);
      renderBgGrid();
      await saveSettings();
    });
...
function renderLanguage() {
  ...
  renderLangGroup("title-lang-group", titleLang, (v) => {
    titleLang = v;
  });
}
...
function renderLayoutTable() {
  ...
  widgetLayout.forEach((w, i) => {
    ...
    colSel.addEventListener("change", () => {
      widgetLayout[i].col = colSel.value;
      saveSettings();
    });
    ...
    ordSel.addEventListener("change", () => {
      widgetLayout[i].order = parseInt(ordSel.value);
      saveSettings();
    });
...
function renderProfileGrid() {
  ...
  profileImages.forEach((img, i) => {
    ...
    removeBtn.addEventListener("click", async (e) => {
      e.stopPropagation();
      await fetch("/api/images/" + profileImages[i].filename, {
        method: "DELETE",
      });
  ...
  if (profileImages.length < 5) {
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      const form = new FormData();
      form.append("image", file);
      const r = await fetch("/api/images/upload?type=profile", {
        method: "POST",
        body: form,
      });
      if (!r.ok) {
        const err = await r.json();
        showUploadError("profile-upload-error", err.error || "upload failed");
        return;
      }
      const record = await r.json();
      profileImages.push(record);
      renderProfileGrid();
      await saveSettings();
    });
    slot.appendChild(fileInput);
...
function renderEngineList() {
  ...
  engines.forEach((eng, i) => {
    ...
    edit.addEventListener("click", () => {
      ...
      editName.style.cssText =
        "background:var(--panel2);border:1px solid var(--border);color:var(--white);font-family:var(--font-pixel);font-size:11px;padding:5px 8px;outline:none;width:100%;";
...
function renderLocation() {
  ...
  if (userLocation.lat && userLocation.lon) {
    ...
    document.getElementById("location-status").textContent = userLocation.city
      ? `using: ${userLocation.city}`
...
document
  .getElementById("location-lookup-btn")
...
document.getElementById("location-lat").addEventListener("input", (e) => {
  userLocation.lat = e.target.value.trim();
});
document.getElementById("location-lon").addEventListener("input", (e) => {
  userLocation.lon = e.target.value.trim();
});
...
function renderBgBlur() {
  renderToggleGroup("bg-blur-group", bgBlur, (v) => {
    bgBlur = v === "true";
  });
}
...
function renderClockDisplay() {
  renderToggleGroup("clock-format-group", clockFormat, (v) => {
    clockFormat = v;
  });
  renderToggleGroup("clock-seconds-group", clockSeconds, (v) => {
    clockSeconds = v === "true";
  });
}
...
/* ── JLPT FILTER ─────────────────────────────────────────────── */
function renderJlpt() {
  renderToggleGroup("jlpt-group", jlptLevel, (v) => {
    jlptLevel = v;
  });
}
...
function renderQuoteList() {
  ...
  // Update count label
  countEl.textContent = atMax
    ? `${quotes.length}/${MAX_QUOTES} · limit reached`
  ...
  quotes.forEach((q, i) => {
    ...
    edit.addEventListener("click", () => {
      ...
      saveEdit.addEventListener("click", () => {
        ...
        customQuotes[i] = {
          text: newText,
          author: editAuthor.value.trim() || "unknown",
        };
        renderQuoteList();
        saveSettings();
      });
...
async function getAllSettings() {
  // fetch current state from the api — single source of truth
  const r = await fetch("/api/data");
  const all = await r.json();
  return {
    nt_theme:         all.nt_theme         ?? currentTheme,
    ...
    nt_quick:         all.nt_quick      ?? [],
    nt_bookmarks:     all.nt_bookmarks  ?? [],
    nt_recent:        all.nt_recent     ?? [],
    nt_notes:         all.nt_notes      ?? "",
  };
}
...
document.getElementById("import-file").addEventListener("change", async (e) => {
  ...
  try {
    ...
    // post everything in one call — handlePostData routes each key correctly
    const r = await fetch("/api/data", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
document.getElementById("reset-btn").addEventListener("click", async () => {
  ...
  try {
    await fetch("/api/data", {
      ...
      body: JSON.stringify({
        nt_theme: "dark",
        nt_engines: ENGINE_DEFAULTS,
        nt_clock_theme: "theme",
        nt_widget_layout: null,
        nt_ui_lang: "en",
        nt_title_lang: "ja",
        nt_location: null,
        nt_username: "",
        nt_font_latin: "share-tech-mono",
        nt_font_jp: "dotgothic16",
        nt_font_clock: "orbitron",
        nt_bg_blur: true,
        nt_clock_format: "24h",
        nt_clock_seconds: true,
        nt_jlpt_level: "all",
        nt_custom_quotes: [],
        nt_quick: [],
        nt_bookmarks: [],
...
async function init() {
  const allData = await fetch("/api/data")
    .then((r) => r.json())
    .catch(() => ({}));
  profileImages = allData.nt_profile_images || [];
  bgImages = allData.nt_bg_images || [];
  engines = allData.nt_engines || ENGINE_DEFAULTS.map((e) => ({ ...e }));
  clockTheme = allData.nt_clock_theme || "theme";
  currentTheme = allData.nt_theme || localStorage.getItem("nt_theme") || "dark";
  // Keep localStorage in sync so settings page preview works
  localStorage.setItem("nt_theme", currentTheme);
  widgetLayout =
    allData.nt_widget_layout || WIDGET_DEFAULTS.map((w) => ({ ...w }));
  // Ensure system-stats is in the layout
  if (!widgetLayout.some((w) => w.id === "system-stats")) {
    widgetLayout.push({
      id: "system-stats",
      col: "right",
      order: 3,
    ...
    widgetLayout.sort((a, b) => a.order - b.order);
  }
  uiLang = allData.nt_ui_lang || "en";
  titleLang = allData.nt_title_lang || "ja";
  userLocation = allData.nt_location || { city: "", lat: "", lon: "" };
  username = allData.nt_username || "";
  fontLatin = allData.nt_font_latin || "share-tech-mono";
  fontJp = allData.nt_font_jp || "dotgothic16";
  fontClock = allData.nt_font_clock || "orbitron";
  bgBlur = allData.nt_bg_blur ?? true;
  clockFormat = allData.nt_clock_format || "24h";
  clockSeconds = allData.nt_clock_seconds ?? true;
  jlptLevel = allData.nt_jlpt_level || "all";
  customQuotes = allData.nt_custom_quotes ?? null;
